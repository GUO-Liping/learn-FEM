...
'''
由于FEM-2Dbar-v1.py中的单元刚度矩阵难以集成，
故在FEM-2Dbar-v1.py版本中将局部坐标系下的K矩阵以节点为单位进行分块，
尝试进一步高效集成为稀疏矩阵
'''
import numpy as np
...
if __name__ == "__main__":
	...
	K_matrix_local_ele_b00 = K_matrix_local_ele.tocsr()[np.arange(0,3),:].tocsc()[:,np.arange(0,3)]
	K_matrix_local_ele_b01 = K_matrix_local_ele.tocsr()[np.arange(0,3),:].tocsc()[:,np.arange(3,6)]
	K_matrix_local_ele_b10 = K_matrix_local_ele.tocsr()[np.arange(3,6),:].tocsc()[:,np.arange(0,3)]
	K_matrix_local_ele_b11 = K_matrix_local_ele.tocsr()[np.arange(3,6),:].tocsc()[:,np.arange(3,6)]

	T_matrix_theta_ele_b00 = T_matrix_theta_ele.tocsr()[np.arange(0,3),:].tocsc()[:,np.arange(0,3)]
	
	K_matrix_global_ele_b00 = K_matrix_global_ele.tocsr()[np.arange(0,3),:].tocsc()[:,np.arange(0,3)]
	K_matrix_global_ele_b01 = K_matrix_global_ele.tocsr()[np.arange(0,3),:].tocsc()[:,np.arange(3,6)]
	K_matrix_global_ele_b10 = K_matrix_global_ele.tocsr()[np.arange(3,6),:].tocsc()[:,np.arange(0,3)]
	K_matrix_global_ele_b11 = K_matrix_global_ele.tocsr()[np.arange(3,6),:].tocsc()[:,np.arange(3,6)]
